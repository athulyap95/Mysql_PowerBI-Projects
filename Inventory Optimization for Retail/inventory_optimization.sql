...
#10.Products with High Demand but Low Stock

SELECT 
  f.product_id, 
  f.store_id,
  SUM(f.demand_price) AS total_demand,
  f.stock_levels
FROM 
  full_inventory_optimization f
GROUP BY 
  f.product_id, f.store_id, f.stock_levels
HAVING 
  SUM(f.demand_price) > f.stock_levels;


#11. Promotion Effectiveness
SELECT promotions, AVG(sales_quantity) AS avg_sales
FROM full_inventory_optimization
GROUP BY promotions
...
SELECT 
  ...
  sales_volume
FROM full_inventory_optimization
WHERE elasticity_index > 1
ORDER BY elasticity_index DESC;

#13. Supplier Lead Time vs Fulfillment Efficiency

SELECT 
  supplier_lead_time,
  AVG(order_fulfillment_time) AS avg_fulfillment_time
FROM full_inventory_optimization
